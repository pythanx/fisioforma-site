...
(function () {
  ...
  // Define larguras para evitar "saltos" e manter autoplay preciso
  function setSizes() {
    const w = carousel.clientWidth;
    items.forEach(el => { el.style.minWidth = w + "px"; });
  ...
  function moveTo(index, animate = true) {
    ...
    const w = carousel.clientWidth;
    if (!animate) track.style.transition = "none";
    // força reflow quando desabilita transition
    // eslint-disable-next-line no-unused-expressions
    track.offsetHeight; 
    track.style.transform = `translateX(-${index * w}px)`;
    if (!animate) {
      // reativa transition depois
  ...
  function start() {
    stop();
    timer = setInterval(() => moveTo(current + 1), INTERVAL);
  }
  function stop() {
  ...
  // Eventos
  dots.forEach((dot, i) => {
    dot.addEventListener("click", () => { moveTo(i); start(); });
  });

  // Pausa no hover (melhor UX)
  ...
  // Pausa quando a aba fica oculta (economia de CPU)
  document.addEventListener("visibilitychange", () => {
    if (document.hidden) stop(); else start();
  });
